...
test('with the Polish-language interface the popup links credits to Filmweb (name search) instead of IMDb', async ({ page }) => {
  await login(page, uniqEmail('filmweb-credits'));
  await enterPicks(page);
  // Switch the interface to Polish, then reload so /api/me re-hydrates the
  // language (the credit links are chosen from getLanguage() at render time).
  ...
  // And no IMDb credit link leaks through for the Polish-language interface.
